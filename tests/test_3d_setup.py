...
class Setup3DFuncTests(unittest.TestCase):
    def test_3d_setup_single_device(self):
        steps = 0.1, 0.2, 0.3
        ...
        assert jnp.allclose(p_x(c), p2_x(c), atol=1e-4)
        assert jnp.allclose(p_y(c), p2_y(c), atol=1e-4)
        assert jnp.allclose(p_z(c), p2_z(c), atol=1e-4)
